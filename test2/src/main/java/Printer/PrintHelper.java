...
public  class   PrintHelper {
    ...
    public int Open(Context context) {
        return  m_print.Open(context);
    }
    public int Close()
    {
        return m_print.Close();
    }
    public int Step(byte bStep)
    {
      return  m_print.Step(bStep);
    }

    public int Unreeling(byte bStep)
    {
        return  m_print.Unreeling(bStep);
    }
    public void GoToNextPage()
    {
        m_print.GoToNextPage();
    }
    //public int PrintImage(short[] data)
    public int PrintImageEx(byte[] data,int nBit)
    {
        return  m_print.PrintImageEx(data,nBit);
    }
    public int PrintString24(byte[] data)
    {
        return  m_print.PrintString24(data);
    }
    public int IsReady()
    {
        return  m_print.IsReady();
    }

    public int SetGrayLevel(byte blevel)
    {
        return  m_print.SetGrayLevel(blevel);
    }
    public int ReadData(byte[] data)
    {
        return  m_print.ReadData(data);
    }
    public int ReadDataEx(byte[] data, int noffset, int ncount)
    {
        return  m_print.ReadDataEx(data,noffset,ncount);
    ...
    public void PrintBitmapAtCenter(Bitmap bm,int labelWidth,int labelHeight) {
        ...
        paint.setColor(Color.WHITE);//it's must need
        canvas.drawRect(new RectF(0,0,bmp.getWidth(),bmp.getHeight()),paint);
        canvas.drawBitmap(bm,(mMaxWidth-bm.getWidth())/2+(mMaxWidth-labelWidth)/2,(labelHeight-bm.getHeight())/2-40,paint);
...
//        else
        {
            ...
            int nwrite = m_print.PrintImageEx(buf,nBit);
        }
